...
namespace Local
{
 ...
 public:
   ...
   bool VerifyDownStreamFilterExecutedPropagation(void) 
   {
     ...
     // extra times
     if ( m_OutputRequestedRegions.size() == 0 )
       {
       itkWarningMacro(<<"Down stream filter didn't execute PropagateRequestedRegion");
       ret = false;
       }
     if (m_OutputRequestedRegions.size() != this->GetNumberOfUpdates() ||
         m_InputRequestedRegions.size() != this->GetNumberOfUpdates()) 
       {       
       itkWarningMacro(<<"Down stream filter didn't execute PropagateRequestedRegion well (may have been called extra times)");
       ret = false;
       }
     return ret;
   }
   ...
   {
     if ( m_InputRequestedRegions.empty() )
       {
       itkWarningMacro(<<"The input filter did not request any regions");
       return false;
       }
   ...
   {
     return VerifyInputFilterMatchedUpdateOutputInformation() &&
       m_NumberOfUpdates == 0;
   }
